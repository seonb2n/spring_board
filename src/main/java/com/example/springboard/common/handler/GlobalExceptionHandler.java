...
public class GlobalExceptionHandler {
    ...
    public CommonResponse<ErrorResponse> handleException(GlobalException exception) {
        // todo logger 처리
        ErrorResponse errorResponse = new ErrorResponse(exception.getErrorType());
        if (exception.getErrorType().getCode() >= 500_000) {
            return CommonResponse.of(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(),
                errorResponse);
        } else if (exception.getErrorType().getCode() >= 400_000) {
            return CommonResponse.of(HttpStatus.BAD_REQUEST.getReasonPhrase(), errorResponse);
        }
        return CommonResponse.of(HttpStatus.BAD_REQUEST.getReasonPhrase(), errorResponse);
    }
}
